...
namespace ThreeNH.Communication.Utility
{
    ...
    {
        public const ushort DefaultInitialRemainder = 0xFFFF;
        ...
        // 计算CRC校验码
        public static ushort CalculateCrc(this IEnumerable<byte> data, ushort initialRemainder = DefaultInitialRemainder)
        {
            const int bitWidth = 16;
            ushort remainder = initialRemainder;
            foreach (var t in data)
            {
                var crcIndex = ((remainder >> (bitWidth - 8)) ^ t) & 0xFF;
                remainder = (ushort)(CrcTable[crcIndex] ^ (remainder << 8));
            }

            return (ushort)(remainder & 0xFFFF);
        }
